...
def token_obrigatorio(f):
    ...
    def decorated(*args,**kwargs):
        ...
        if not token:
            return jsonify({'menagem': 'Token não foi incluido"'}, 401)
        try:
...
def obter_autor_por_id(autor,id_autor):
    ...
    if not autor:
        return jsonify('Autor n encontrado!')
    autor_atual = {}
    autor_atual['id-autor'] = autor.id_autor
    autor_atual['nome'] = autor.nome
    autor_atual['email'] = autor.email
...
def excluir_autor(autor,id_autor):
    ...
    if not autor_existente:
        return jsonify({'mensagem': 'Esse autor n foi encontrado'})
    db.session.delete(autor_existente)
    db.session.commit()
    return jsonify({'mensagem': 'Autor excluido com sucesso!'}, 202)
